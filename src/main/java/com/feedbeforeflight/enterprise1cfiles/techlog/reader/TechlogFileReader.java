...
public class TechlogFileReader implements AutoCloseable {
    ...
    private final TechlogFile techlogFile;
    private int skippedLines = 0;

    public TechlogFileReader(TechlogFile techlogFile) {
        this.techlogFile = techlogFile;
    }
    ...
    public void openFile() throws IOException {
        reader = new LineNumberReader(new FileReader(techlogFile.getPath().toString(), StandardCharsets.UTF_8));
    ...
    public void skipToLine(int linesCount) {
        if (reader != null) {
            try {
                ...
                log.error("Exception while skipping lines at file " + techlogFile.getId(), e);
            }
        }
    }
